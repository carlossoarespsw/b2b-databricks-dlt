...
CONFIG_PATH = f"/Workspace/Repos/sp_b2b_ops_bot/b2b-databricks-dlt-{ENVIRONMENT}/config/tables_b2b_public.yaml"
...
def ingest_batch(table_conf, start_date, end_date):
    ...
    t_wm = table_conf.get("watermark")  # Retorna None se não existir
    ...
    if t_wm:
        ...
        # Sem watermark: carrega todos os dados
    ...
    if estimated_rows == 0:
        print(f"Batch vazio. Tempo leitura: {read_time:.2f}s")
        return 0

    num_partitions = min(max(8, estimated_rows // 1_000_000), 128)
    df = df.repartition(num_partitions)

    if t_wm and ("date" in t_wm or "created" in t_wm or "last_modified" in t_wm):
        ...
        # Sem watermark: sem particionamento por data
        partition_cols = []

    df = df.withColumn("_ingestion_ts", F.current_timestamp())

    t2 = pytime.time()
    write_builder = df.write.format("delta") \
        .mode("append") \
        .option("mergeSchema", "true")
    
    if partition_cols:
        write_builder = write_builder.partitionBy(*partition_cols)
    
    write_builder.saveAsTable(target_table)
    t3 = pytime.time()
    write_time = t3 - t2
    ...
    try:
        ...
        print(f" Não foi possível obter tamanho dos arquivos Delta: {e}")
...
if not heavy_tables:
    ...
    for table_conf in heavy_tables:
        ...
        if t_wm:
            # Tabela com watermark: usa checkpoint
            if last_date:
            ...
            # Tabela sem watermark: carga completa
            print(f"⚠️ Nenhum campo watermark definido. Realizando carga completa.")
        ...
        if t_wm:
            # Com watermark: processa em batches com checkpoint
            p_bar = TableProgressBar(t_name, start_date, end_date)
            current_date = start_date
            total_rows = 0
            batch_count = 0
            print(f"\n🚀 Iniciando Processamento em Batches...")
            ...
            # Sem watermark: carga completa única
            print(f"\n🚀 Iniciando Carga Completa (sem watermark)...")
            try:
                success = ingest_batch(table_conf, start_date, end_date)
                print(f"\n📊 Resumo {t_name}:")
                print(f"   Carga completa realizada com sucesso")
            except Exception as e:
                print(f"      ❌ Falha crítica: {e}")